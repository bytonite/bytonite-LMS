...
### Версия 1.21.0
- Добавлены кастомные стилизованные скроллбары для приложения (светлая/темная темы).
- Исправлен баг "Grid Blowout" при сплит-экране (сетка больше не разъезжается).
- Добавлена история изменений (Undo/Redo, Ctrl+Z / Ctrl+Y) и возможность удаления (Delete) блоков в режиме дизайна (Preview).
- Исправлен баг логики Drag & Drop: предотвращено создание flex-row при добавлении ячеек внутрь сетки dashboard-grid (теперь колонки встают правильно).
- Улучшен интерфейс поиска: добавлены сниппеты (строки с текстом) для наглядности совпадений.
- Поиск больше не сканирует служебные папки (node_modules и пр.), что исключает появление дубликатов.
