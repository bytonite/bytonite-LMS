...
### Версия 1.22.0
- **Drag & Drop блоков кода:** Исправлено сохранение многострочного форматирования (переносов строк) при перетаскивании блоков кода внутрь ячеек сетки.
- **Свойства сеток и коллаутов:** Убрано жесткое ограничение скругления углов, теперь можно задавать пользовательский `border-radius` для коллаутов, изображений и видео внутри сеток.
- **Прозрачные цвета:** В правую панель свойств (Properties) добавлена специальная кнопка для установки прозрачного цвета (текста, фона и границ).
- **Позиционирование ползунков:** Зеленый квадрат для перетаскивания (drag handle) возвращен на правильное место внутри ячеек, сеток и коллаутов, а для блоков кода позиционируется отдельно (слева от кнопки копирования), чтобы не перекрывать интерфейс.
- **Панель шаблонов:** Шаблон «Блок кода» теперь доступен во вкладке «Коллауты».
- **Улучшение правой панели:** Полностью переработана механика ввода размеров (Ширина, Высота, Padding, Margin):
  - Поля теперь показывают актуальные (вычисленные) стили, а не пустые значения.
  - Сохранение размеров работает как по нажатию `Enter`, так и по клику мыши вне поля.
  - Автоматическое добавление `px` при вводе только чисел.
  - Автообновление значений при переключении между контейнером (Box) и контентом (Content).
